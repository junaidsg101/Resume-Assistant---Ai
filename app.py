...
# Safe attempt to read Streamlit secrets without triggering StreamlitSecretNotFoundError
try:
    # st.secrets behaves like a dict when secrets exist; get() returns None if key missing.
    api_key = st.secrets.get(GEMINI_ENV_NAME)
except Exception:
    # st.secrets may not exist in some local setups; ignore and fallback to .env
    api_key = None

if not api_key:
    # fallback to dotenv / environment for local dev
...
if not api_key:
    st.warning(
        ...
        "For local testing, create a `.env` file with the same variable name or a `.streamlit/secrets.toml` file (both are gitignored)."
    )

# Instantiate Gemini client (client will handle None api_key defensively)
...
if st.button("Evaluate Resume"):
    ...
    with left:
        ...
        ui_components.plot_score_charts(result.get("scores", {}), weights)
        overall_score = scorer.compute_weighted_score(result.get("scores", {}), weights)
        ...
        weakest = min(result.get("scores", {}).items(), key=lambda kv: kv[1])[0] if result.get("scores") else None
        if weakest:
            st.markdown(f"### Improve: {weakest}")
            st.write(result.get("justifications", {}).get(weakest, ""))
    ...
    with right:
        ...
        for k, v in result.get("scores", {}).items():
            ...
            st.write(result.get("justifications", {}).get(k, ""))
            st.markdown(f"*Improvement tip:* {result.get('tips', {}).get(k, '')}")
...
# --- Header ---
st.title("AI Resume Assistant")
st.write("Evaluate and improve resumes with domain-specific guidance powered.")
